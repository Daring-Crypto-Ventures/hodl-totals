...
export function test9CostBasis(): unitTestWrapper {
    return (): void => {
        ...
        const TestRun = function (round): void {
            ...
            const soldNote1 = annotations.shift() as string[];
            assert(soldNote1?.[0], 'D28', `Round ${round} Test for Lot Sold Hint : Hint Anchor point on row 28`);
            ...
            const soldNote2 = annotations.shift() as string[];
            assert(soldNote2?.[0], 'D29', `Round ${round} Test for Lot Sold Hint : Hint Anchor point on row 29`);
